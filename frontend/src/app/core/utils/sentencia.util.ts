...
export function sentenciaSegunMotorValidator(grupo: AbstractControl): ValidationErrors | null {
  ...
  const motor = grupo.get('motor')?.value as MotorSentencia | null;
  const mensaje = advertenciaSentencia(sentencia, motor);
  return mensaje ? { sentenciaNoPermitida: mensaje } : null;
}
